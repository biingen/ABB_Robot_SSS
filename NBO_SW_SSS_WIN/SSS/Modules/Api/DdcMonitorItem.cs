...
namespace ModuleLayer.Api
{
	...
	internal class DdcMonitorItem : MonitorItem
	{
		...
		{
			var result = MonitorConfiguration.GetBrightness(_handle, (uint)brightness, _useHighLevel);
            /*
			if ((result.Status == AccessStatus.Succeeded) && (_minimum < _maximum) && (minimum <= current) && (current <= maximum))
			{
	...
	internal class SafePhysicalMonitorHandle : SafeHandle
	{
        ...
        protected override bool ReleaseHandle() => MonitorConfiguration.DestroyPhysicalMonitor(handle);
        /*
